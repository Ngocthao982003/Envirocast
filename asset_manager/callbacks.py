...
def create_envirocast_asset_browser_entry():
    context = bpy.context
    ...
    if library:
        ...
        if not cats_path.exists():
            with open(cats_path, "w") as f:
                ...
                f.write(
                    '# Other lines are of the format "UUID:Path to Asset:Simple Catalog Name"\n'
                )
                f.write("\n")
                f.write("VERSION 1\n")
                f.write("\n")

                f.write(f"{default.GROUP26_CATALOG_ID}:Group26:Group26\n")
                for cat in catalogs:
                    id = getattr(default, f"{cat}_CATALOG_ID")
                    f.write(f"{id}:Group26/{catalogs[cat]}:Group26-{catalogs[cat]}\n")
...
def create_library(self, Dephsgraph):
    ...
    if "Group26" not in bpy.context.preferences.filepaths.asset_libraries.keys():
        creating = True
        bpy.ops.preferences.asset_library_add()
        new_library: UserAssetLibrary = (
            bpy.context.preferences.filepaths.asset_libraries[-1]
        )
        new_library.name = "Group26"
        new_library.path = browser_library.as_posix()
        create_envirocast_asset_browser_entry()
        load_library(True)
        refresh_library()
        creating = False
    else:
        library = bpy.context.preferences.filepaths.asset_libraries["Group26"]
...
@persistent
def clear_group26_duplicates(self, dephs):
    try:
        entry = next(
            (idx, ent)
            for idx, ent in enumerate(bpy.context.preferences.filepaths.asset_libraries)
            if ent.name.startswith("Group26.")
        )
    except Exception:
        debug("No Duplicates Found.")
        return
    debug(f"Removing Group26 filepath duplicates {entry[1].name}")
    bpy.ops.preferences.asset_library_remove(index=entry[0])
    clear_group26_duplicates(None, None)


@persistent
def check_envirocast_asset_browser_entry(self, context: bpy.types.Context):
    add_callback(create_envirocast_asset_browser_entry)


def register():
    bpy.app.handlers.load_post.append(check_envirocast_asset_browser_entry)
    bpy.app.handlers.load_post.append(clear_group26_duplicates)
    bpy.app.handlers.load_post.append(create_library)


def unregister():
    bpy.app.handlers.load_post.remove(check_envirocast_asset_browser_entry)
    bpy.app.handlers.load_post.remove(clear_group26_duplicates)
