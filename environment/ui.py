...
class EnvironmentCreatorPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_EnvironmentPanel"
    bl_label = "Environment Creator"
    bl_options = {"DEFAULT_CLOSED"}
    bl_order = 2
    bl_parent_id: str = "ENVIROCAST_PT_scatter"
    ...
    def draw(self, context: bpy.types.Context):
        ...
        row.prop(environment_creator, "preview")
        row.operator(PreviewOperator.bl_idname, text="", icon="FILE_IMAGE")

        if environment_creator.preview:
            col.box().template_icon(
                icon_value=get(environment_creator.preview).icon_id, scale=10
            )
        ...
        if environment_creator.show_systems:
            ...
            col = col.box().column()
            for item in scatter_surface.envirocast.scatter_items:
...
class EnvironmentPropsPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_EnvironmentProps"
    ...
    def draw_header_preset(self, context: Context):
        ...
        layout.prop(
            ss.envirocast.environment_props,
            "editing",
            text="",
            icon="GREASEPENCIL",
            toggle=True,
    ...
    def draw(self, context):
        ...
        layout.use_property_decorate = not ss.envirocast.environment_props.editing
        ...
        for prop in props:
            ...
            try:
                ...
                continue
            split = layout.split(
                factor=0.8 if ss.envirocast.environment_props.editing else 1, align=True
            )
            ...
            if ss.envirocast.environment_props.editing:
                row = split.row(align=True)
                row.alignment = "RIGHT"
                row.separator(factor=0.3)
                op = row.operator(
                    ReorderPropertyOperator.bl_idname, text="", icon="TRIA_UP"
                )
                op.prop_idx = idx
                op.direction = "UP"
                op.tooltip = "UP"
                op = row.operator(
                    ReorderPropertyOperator.bl_idname, text="", icon="TRIA_DOWN"
                )
                op.prop_idx = idx
                op.direction = "DOWN"
                op.tooltip = "DOWN"
                op = row.operator(
                    RenameProperty.bl_idname, text="", icon="GREASEPENCIL"
                )
                op.prop_idx = idx
                row.separator(factor=0.3)
                op = row.operator(
                    RemovePropertyOperator.bl_idname, text="", icon="X", emboss=False
                )
                op.prop_idx = idx
