...
if TYPE_CHECKING:
    from ..common.prefs import EnviroCastPreferences
...
def get_preferences(source: Context = None) -> "EnviroCastPreferences":
...
def get_wm_props(source: Context = None) -> "WindowManagerProps":
    source = source if source else bpy.context
    return source.window_manager.envirocast


def get_scene_props(source: Union[Context, Depsgraph] = None) -> "SceneProps":
    source = source if source else bpy.context
    return source.scene.envirocast


def get_scatter_surface(
    source: Union[Context, Depsgraph] = None,
) -> Union[Object, None]:
    scene_props = get_scene_props(source)
    return scene_props.scatter_surface


def get_scatter_props(
    source: Union[Context, Depsgraph] = None,
) -> Union["ObjectProps", None]:
    scatter_surface = get_scatter_surface(source)
    return scatter_surface.envirocast if scatter_surface else None


def get_scatter_item(
    source: Union[Context, Depsgraph] = None,
) -> Union["ScatterItemProps", None]:
    scatter_props = get_scatter_props(source)
    if scatter_props and scatter_props.scatter_index in range(
        len(scatter_props.scatter_items)
    ):
...
def get_nodes_modifier(
    source: Union[Context, Depsgraph, Object] = None,
) -> Union[NodesModifier, None]:
    scatter_system = (
        source if isinstance(source, Object) else get_scatter_system(source)
    )
    if scatter_system:
        return scatter_system.modifiers.get("EnviroCastGeometryNodes")


def get_system_tree(
    source: Union[Context, Depsgraph, Object] = None,
) -> Union[bpy.types.GeometryNodeTree, None]:
...
def get_node_tree(
    source: Union[Context, Depsgraph, Object] = None,
) -> Union[GeometryNodeTree, None]:
    nodes_modifier = get_nodes_modifier(source)
    if nodes_modifier and nodes_modifier.node_group:
        node = nodes_modifier.node_group.nodes.get("EnviroCast")
        if node is None:
...
def get_instance_col(
    ...
    if node_tree:
        ...
        for effect in node.node_tree.envirocast.effects:
            effect_id = (
                effect.get_effect_id() if effect.get_effect_id() else effect.effect_id
            )
            if effect_id == "system.instance_collection":
                effect_node = node.node_tree.nodes[
                    effect.name + "_group"
                ].node_tree.nodes.get("effect")
                if effect_node is not None:
                    col = (
                        node.node_tree.nodes[effect.name + "_group"]
                        .node_tree.nodes["effect"]
                        .inputs["Collection"]
                        .default_value
                    )
                    return col
...
def get_scatter_system_effects(item, is_old=False):
    ...
    for category in categories:
        ...
        effect_list = node.node_tree.envirocast.effects
        for effect in effect_list:
            effect_data, dependency = get_effect_data(effect, is_old, envirocast_assets)
    ...
    return distribution, scale, rotation, geometry, envirocast_assets, dependencies
...
def get_effect_data(
    ...
    effect_version = (
        effect.get_effect_version() if effect.get_effect_version() else effect.version
    )
    ...
    for inp in effect.effect_node.inputs.values():
        if (
            len(inp.links) == 0
            and inp.bl_idname not in NON_PRIMITIVE_SOCKETS
            and not inp.name.startswith("_")
        ):
            ...
            if inp.type in ["OBJECT", "COLLECTION", "IMAGE", "MATERIAL"]:
                ...
                if (
                    inp.type == "OBJECT"
                    and inp.default_value
                    and not inp.default_value.envirocast.is_envirocast_system
                ):
                    ...
                    params[str(idx)]["is_envirocast_asset"] = (
                        inp.default_value.envirocast.is_envirocast_asset
                        if inp.default_value
                        else False
                    )
                    if (
                        inp.default_value
                        and inp.default_value.envirocast.is_envirocast_asset
                    ):
                        asset_id = (
                            inp.default_value.envirocast.asset_id
                            if inp.default_value.envirocast.asset_id
                            else inp.default_value.name.split(":")[-1]
                        )

                        library = bpy.context.window_manager.envirocast.library
                        asset = [
                            ass for ass in library.assets if ass.asset_id == asset_id
                        ]
                        ...
                        if asset:
                            ...
                            params[str(idx)]["lod"] = ob_name_split[-1].removeprefix(
                                "LOD"
                            )

                            envirocast_assets.append(asset_id)

                    elif (
                        inp.default_value
                        and not inp.default_value.envirocast.is_envirocast_asset
                    ):
    ...
    if effect.mix_node:
        ...
        layer_params = None
    nodes = get_node_properties(effect.effect_node.node_tree, envirocast_assets)
...
def get_nodes_from_node_tree(node_tree: bpy.types.GeometryNodeTree, nodes: set):
    for node in node_tree.nodes:
        ...
        elif node.envirocast.display_in_effect:
            nodes.add(node)


def get_node_properties(node_tree: bpy.types.GeometryNodeTree, envirocast_assets: list):
    ...
    for node in nodes:
        if not node.envirocast.display_in_effect:
            continue
        data = {}
        ...
        data["params"] = get_params(node, envirocast_assets=envirocast_assets)
...
def get_params(node, envirocast_assets) -> dict:
    params = {}
