        ...
        for item in scatter_items:
            for category in default.EFFECT_CATEGORIES:
                ...
                for effect in effects:
                    if effect.group_node.node_tree.envirocast.category == ["NONE"]:
                        ...
                        if effect_item:
                            effect.group_node.node_tree.envirocast.category = (
                                effect_item.categories
                            )
                            effect.group_node.node_tree.envirocast.icon = effect_item.icon
...
def node_deleted_handler(self, context: bpy.types.Context):
    ...
    if selected and selected.group_node is None:
        index = node.node_tree.envirocast.effects.find(selected.name)
        node.node_tree.envirocast.effects.remove(index)
