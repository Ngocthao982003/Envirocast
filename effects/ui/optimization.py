...
class OptimizationPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_optimization"
    ...
    def poll(cls, context):
        ...
        return True and scatter_props  # and len(scatter_props.scatter_items) > 0

    def draw_header(self, context: bpy.types.Context):
        self.layout.label(icon="MOD_REMESH")

    def draw(self, context):
        envirocast = get_scene_props(context)
        layout = self.layout
        col = layout.column()
        col.use_property_split = True
        col.prop(envirocast, "viewport_display_percentage", slider=True)


class CameraCullingPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_camera_culling"
    bl_label = "Camera Culling"
    bl_order = 112
    bl_parent_id: str = "ENVIROCAST_PT_optimization"
    bl_options = {"DEFAULT_CLOSED"}

    def draw_header(self, context: bpy.types.Context):
        envirocast = get_scene_props(context)
        layout = self.layout
        layout.prop(envirocast.camera_culling, "use", text="")
    ...
    def draw(self, context):
        ...
        cam_seletor_col.prop_search(
            camera_culling, "camera", bpy.context.scene, "objects"
        )
...
class ProxySettingsPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_proxy_settings"
    ...
    def draw_header(self, context: bpy.types.Context):
        ...
        layout = self.layout
        # layout.prop(envirocast.proxy_settings, "proxy_method", text="")
        layout.separator()

    def draw(self, context):
        prefs = get_preferences(context)
        envirocast = get_scene_props(context)
        proxy_settings: ProxySettingsProps = envirocast.proxy_settings
...
class GlobalSettingsPanel(BasePanel):
    bl_idname = "ENVIROCAST_PT_global_settings"
